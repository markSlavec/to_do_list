...
void validation_user_input(value *user_input, typedata fuctuio_data) {
    ...
    while (flag) {
        switch (fuctuio_data) {
            ...
            case CHAR:
                if (fscanf(stdin, " %[^\n]", user_input->new_task) == 1) { // Добавлен пробел перед %[^\n] для пропуска пробелов
                    flag = 0;
                    fflush(stdin); // fflush(stdin) не работает для ввода
                    break;
                }
    ...
    while (flag) {
        ...
        switch (user_input_option.id) {
            case 1: {
                ...
                validation_user_input(&user_input, CHAR); // Исправлено: user_input_option на user_input
            ...
            case 3: {
                ...
                validation_user_input(&user_input, INT); // Исправлено: добавлена валидация ввода
...
void add_new_task(char* name_new_task, buffer* buffer_data) {
    if (buffer_data->len_buffer <= buffer_data->task_count) { // Исправлено условие
        buffer_data->len_buffer *= 2;
        buffer_data->buffer_tasks = realloc(buffer_data->buffer_tasks, sizeof(task) * buffer_data->len_buffer);
    }
...
void delete_task(int id, buffer* bd) {
    if (id > 0 && (size_t)id <= bd->task_count) {
        for (size_t i = id - 1; i < bd->task_count - 1; i++) { // Исправлено условие цикла
